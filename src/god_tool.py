...
def main():
    ...
    if args.command == "connect":
        ...
        try:
             ...
             # Security Check: Validate partition name to prevent command injection
             if not re.match(r'^[a-zA-Z0-9_]+$', partition_name):
                 print(f"[-] Error: Invalid partition name derived from filename: '{partition_name}'. Only alphanumeric characters and underscores are allowed.")
                 sys.exit(1)
