...
from src.modem import backup_efs, wipe_efs, enable_diag_mode
...
class TestModem(unittest.TestCase):
    ...
    @patch("src.modem.os.path.exists")
    @patch("src.modem.calculate_local_md5")
    def test_backup_efs_md5_mismatch(self, mock_calc_md5, mock_exists, mock_run):
        # Setup mocks
        mock_exists.return_value = True # Directory exists
        mock_calc_md5.return_value = "local_md5_mismatch"

        # Mock subprocess.run to return a different MD5
        def side_effect(*args, **kwargs):
            cmd = args[0]
            if "md5sum" in cmd:
                return MagicMock(stdout="remote_md5_mismatch  /sdcard/modemst1.img\n")
            return MagicMock(stdout="")
        ...
        self.assertIn("MD5 mismatch for modemst1! Backup corrupted.", str(context.exception))
